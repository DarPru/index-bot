...
# перевод логов ошибок с консоли на файл
f = open('log.txt', 'w')
...
def write_result(work_type, url, date):
    ...
    if work_type == 'txt_file':
        ...
        with open(f'logs/log_{now.day}-{now.month}-{now.year}.txt', 'a', encoding='utf-8') as result_file:
...
print("Отправлено на индексацию: " + str(count_urls) + " шт.", file=f)
